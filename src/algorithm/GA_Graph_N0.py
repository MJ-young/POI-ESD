...
class Individual:
    ...
    def calculate_fitness(self, environment, service):
        ...
        # cover_quality = sum(environment.pois[pid].correlation * (environment.gamma ** h) for pid,h in self.h_hop_pois.items() if h>0) / 4
        
        # cover_quality = sum(environment.pois[pid].correlation * (environment.gamma ** h) for pid,h in self.h_hop_pois.items())
        cover_quality = sum(environment.pois[pid].correlation * (environment.gamma ** h) for pid,h in self.h_hop_pois.items()) / 150
...
def GA_Graph_N0(environment, service):
